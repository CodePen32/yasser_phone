...
export function HeroSection({ settings }: HeroSectionProps) {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 max-w-[1500px] mx-auto px-4 sm:px-6 py-10 flex flex-col md:flex-row items-center gap-8">

        {/* Left: product showcase cards */}
        <div className="hidden md:grid grid-cols-2 gap-3 shrink-0 w-80">
          {[
            { emoji: '📱', label: 'آيفون 15 Pro', sub: 'متوفر الآن', color: '#1c1c2e' },
            { emoji: '⌚', label: 'Apple Watch', sub: 'ساعات ذكية',  color: '#1a2a1a' },
            { emoji: '🎧', label: 'AirPods Pro',  sub: 'سماعات أصلية', color: '#1a1a2e' },
            { emoji: '🔋', label: 'باور بانك',    sub: 'شحن سريع',   color: '#2a1a1a' },
          ].map((item) => (
            <a
              key={item.label}
              href="/products"
              className="rounded-xl p-3 flex flex-col items-center justify-center gap-2 hover:opacity-90 transition-opacity aspect-square"
            ...
            >
              ...
              <div className="text-center">
                ...
                <div className="text-[10px]" style={{ color: 'rgba(255,255,255,0.5)' }}>{item.sub}</div>
              </div>
            </a>
          ))}
        </div>
        ...
        <div className="flex-1 text-center md:text-end">
          ...
          >
            <span className="w-1.5 h-1.5 rounded-full bg-amber-400 inline-block anim-float" />
            عروض هذا الأسبوع
          </div>

          <h1
            className="font-display font-black leading-tight mb-3"
            style={{ color: 'white', fontSize: 'clamp(1.8rem, 4vw, 3rem)' }}
          >
            عروض الهواتف في {settings.city}
          </h1>

          <p className="text-sm sm:text-base mb-3 max-w-md mx-auto md:mr-0 md:ml-auto" style={{ color: 'rgba(255,255,255,0.7)' }}>
