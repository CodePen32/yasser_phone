  ...
  return (
    <div className="card-sheen group bg-[var(--c-surface)] rounded-xl border border-[var(--c-border)] hover:border-[var(--c-accent)] hover:shadow-lg hover:shadow-amber-500/10 transition-all duration-300 flex flex-col overflow-hidden">
      ...
      <div className="flex flex-col flex-1 p-2 sm:p-3 gap-1.5">
        {/* Brand */}
        {product.brand && (
          <span className="text-[10px] sm:text-[11px] text-[var(--c-muted)] font-medium uppercase tracking-wide num-latin">
            {product.brand.name}
          </span>
        )}
        ...
        <Link href={`/products/${product.slug}`}>
          <h3 className="text-xs sm:text-sm font-semibold text-[var(--c-text)] line-clamp-2 leading-snug hover:text-[var(--c-link)] transition-colors">
            {product.name_ar}
          </h3>
        </Link>

        {/* Rating — hidden on smallest screens to save space */}
        {product.rating && (
        ...
        <div className="mt-auto pt-1">
          <div className="flex items-baseline gap-1 sm:gap-2 flex-wrap">
            <span className="text-sm sm:text-lg font-bold text-[var(--c-price)] num-latin ltr-value">
              {product.price.toLocaleString()}
            </span>
            <span className="text-[10px] sm:text-xs font-semibold text-[var(--c-muted)]">{product.currency}</span>
            {product.old_price && (
              <span className="text-[10px] sm:text-xs text-[var(--c-muted)] line-through num-latin ltr-value">
          ...
          {product.storage && (
            <p className="text-[10px] sm:text-[11px] text-[var(--c-muted)] mt-0.5 num-latin">
        ...
        <a
          href={waUrl}
          onClick={handleOrder}
          className="mt-1.5 w-full flex items-center justify-center gap-1.5 wa-grad text-white text-xs sm:text-sm font-semibold py-1.5 sm:py-2 rounded-lg hover:opacity-90 active:scale-[0.98] transition-all"
        >
